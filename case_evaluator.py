...
class CaseEvaluator:
    ...
    def trains_next(self):
        ...
        result = list(set(available_train_tags).intersection(set(self.map.available_tags)))
        return result
        ...
        for train in [1, 2, 3, 4]:
            ...
            if start_position != 'collided':
                while moving == 1:
                    ...
                    if position in case['map'].tracks.keys():
                        ...
                        if through in case['map'].train_positions.values():
                            colliding_train = [x for x in case['map'].train_positions
                                               if case['map'].train_positions[x] == through][0]
                            self.map.vprint('Trains {} and {} have collided!'.format(train, colliding_train))
                ...
                if start_position != end_position:
                    case['map'].summary.append('Train {} moved from {} to {} in {} moves'.format(
    ...
    def try_update_paths(self, case):
        ...
        for direction in self.map.directions:
            neighbour_tag = tools.move_a_tag(case['tag'], self.map.directions[direction])
            if neighbour_tag in used_tags:
                ...
                for gate_pair in all_gates:
                    ...
                    end_from = neighbour_tag + str(tools.get_other(neighbour_card, gate_pair[1]))
